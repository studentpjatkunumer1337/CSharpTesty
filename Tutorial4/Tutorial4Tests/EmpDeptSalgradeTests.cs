...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        List<Emp> result = (from e in emps
            where (from d in depts where d.Loc == "CHICAGO" select d.DeptNo).Contains<int>(e.DeptNo)
            select e).ToList();
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        ...
        var result = emps.GroupBy(e => e.DeptNo, e => e.Sal)
            .Select(g => new
            {
                DeptNo = g.Key,
                AvgSal = g.Average()
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = from e in emps
            where e.Sal > (from j in emps where j.DeptNo == e.DeptNo select j).Average(e => e.Sal)
            select e.EName;
